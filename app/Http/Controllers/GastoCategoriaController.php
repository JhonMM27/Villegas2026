...
class GastoCategoriaController extends Controller
{
    ...
    {
        $this->middleware('can:gasto_categorias_list')->only(['index']);
        $this->middleware('can:gasto_categorias_create')->only(['store']);
        $this->middleware('can:gasto_categorias_edit')->only(['show', 'update']);
        $this->middleware('can:gasto_categorias_delete')->only(['destroy']);
    }

    public function index(Request $request)
    {
        if ($request->ajax()) {
            $data = GastoCategoria::select(['id', 'nombre', 'activo']);

            return DataTables::of($data)
                ->addColumn('action', function ($row) {
                    $editButton = '';
                    if (auth()->user()->can('gasto_categorias_edit')) {
                        $editButton = view('components.button-edit', ['id' => $row->id])->render();
                    }
                    $deleteButton = '';
                    if (auth()->user()->can('gasto_categorias_delete')) {
        ...
        return view('gasto-categorias.index');
    }
    ...
    {
        try {
            $registro = GastoCategoria::findOrFail($id);
            $registro->delete();
    ...
    {
        $query = GastoCategoria::select('id', 'nombre')
            ->where('activo', true);
