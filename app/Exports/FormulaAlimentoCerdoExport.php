...
class FormulaAlimentoCerdoExport implements WithEvents
{
    protected FormulaAlimentoCerdo $formula;

    protected array $data;

    public function __construct(int $formulaId)
    {
        $formula = FormulaAlimentoCerdo::with('detalles.ingrediente')->findOrFail($formulaId);
        $this->formula = $formula;
        $this->data = app(FormulaAlimentoCerdoService::class)->calcular($formula);
    }
    ...
    private function buildSheet(\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $sheet): void
    {
        ...
        $sheet->setCellValue("A{$row}", 'FÓRMULA DE ALIMENTO — CERDOS');
        ...
        $sheet->getStyle("A{$row}")->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('FFFCE4D6');
        $row++;
        ...
        $nutRows = [
            ...
            ['EM (Mcal/kg)', $nutricion['em']],
            ['Grasa', $nutricion['grasa']],
            ['Fibra', $nutricion['fibra']],
            ['FDN', $nutricion['fdn']],
            ['FDA', $nutricion['fda']],
            ['Calcio', $nutricion['calcio']],
            ['Fósforo', $nutricion['fosforo']],
            ['Magnesio', $nutricion['magnesio']],
            ['Almidón', $nutricion['almidon']],
            ['Azúcar', $nutricion['azucar']],
            ['Ceniza', $nutricion['ceniza']],
            ['Lactosa', $nutricion['lactosa']],
            ['Lisina', $nutricion['lisina']],
            ['Metionina', $nutricion['metionina']],
            ['Treonina', $nutricion['treonina']],
        ];
