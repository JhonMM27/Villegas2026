...
<body>
    ...
    <div class="page-header">
        <table>
            ...
            <tr>
                <td>
                    Compras Detalladas Agrupadas por Proveedor
                    {{ \Carbon\Carbon::parse($fechaInicio)->format('d/m/Y') }}
                    al
    ...
    <table class="reporte">
            ...
            @forelse($reportesOrdenados as $item)
                ...
                @if($proveedorActual !== null && $proveedorActual !== $item->proveedor_nombre)
                    <tr class="subtotal-proveedor">
                        <td class="text-right">Subtotal {{ $proveedorActual }} ({{ $contadorCompras }} compras):</td>
                        <td colspan="6"></td>
                        <td class="text-right">{{ number_format($subtotalIngKg, 2) }}</td>
        ...
        <tfoot>
            <tr class="total-row linea">
                <td colspan="7" class="text-right"><strong>TOTAL GENERAL ({{ $totalCompras }} compras):</strong></td>
                <td class="text-right"><strong>{{ number_format($totalIngKg, 2) }}</strong></td>
