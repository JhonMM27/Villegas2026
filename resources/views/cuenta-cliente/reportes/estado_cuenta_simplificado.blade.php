...
<table class="report">
    ...
    <colgroup>
        ...
        <col style="width: 45px;">  <!-- Fecha Venta (más angosta) -->
...
@php
    ...
    foreach(($ventas ?? collect()) as $__v) {
        ...
        foreach($__pagos as $__p) {
            $key = $__p->provisional_id
                ?? $__p->provisionalId
                ?? (trim((string)($__p->numero_recibo ?? '')) . '|' . (string)($__p->fecha_provisional ?? ''));
...
@forelse($ventas as $v)
    ...
    @if($nPagos === 0)
        <tr class="row-venta">
            ...
            <td class="num"><span class="muted">{{ $fechaVentaFmt }}</span></td>
        </tr>
    @else
        @foreach($pagos as $i => $p)
            @php
                ...
                // lógica original de acumulado (se mantiene)
                $saldoAcum -= ($saldoVenta - $saldoDespues);
                $saldoVenta = $saldoDespues;

                $esPrimero = ($i === 0);

                $provKey = $p->provisional_id
                    ?? $p->provisionalId
                    ?? (trim((string)($p->numero_recibo ?? '')) . '|' . (string)($p->fecha_provisional ?? ''));
                ...
                if(!isset($provUsado[$provKey])) {
                    ...
                    $provFechaFmt = !empty($provFecha) ? \Carbon\Carbon::parse($provFecha)->format('d/m/Y') : $fechaProvFmt;
                }
            @endphp
            ...
            <tr class="row-pago">
                ...
                <td class="num">
                    @if($mostrarProv)
                        <span style="color:#b30000; font-weight:700;">{{ number_format($provTotal, 2) }}</span>
                    @endif
                </td>
                <td class="num">
                    @if($mostrarProv)
