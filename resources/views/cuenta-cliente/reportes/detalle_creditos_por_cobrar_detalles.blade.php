...
<html>
@include('pdf.styles', [
  'title' => "Detalle créditos por cobrar ({$ini->format('d/m/Y')} al {$fin->format('d/m/Y')})"
...
<body>
    ...
    <div class="page-header">
        <table>
            ...
            <tr>
                <td>
                    Reporte: Detalle créditos por cobrar ({{ $ini->format('d/m/Y') }} al {{ $fin->format('d/m/Y') }})
                </td>
                <td class="text-right">
    ...
    <table class="reporte">
        ...
        <tbody>
            ...
            @forelse($reportes as $r)
                ...
                {{-- Cerrar venta anterior --}}
                @if($ventaActual !== null && $isNewVenta)
            ...
            {{-- Cerrar última venta + Total general --}}
            @if(count($reportes) > 0)
                <tr class="subtotal-row">
                    <td colspan="2">
                        Abono: S/. {{ number_format($abonoActual,2) }}
