...
<html>
@include('pdf.styles', [
    'title' => "Reporte: Ventas detallado ({$ini->format('d/m/Y')} al {$fin->format('d/m/Y')})"
...
<body>
    ...
    <div class="page-header">
        <table>
            ...
            <tr>
                <td>
                    Reporte: Ventas detallado ({{ $ini->format('d/m/Y') }} al {{ $fin->format('d/m/Y') }})
                </td>
                <td class="text-right">
    ...
    <table class="reporte">
        ...
        <tbody>
            @php
                ...
                // Guardar abono/saldo de la venta actual para imprimir en el subtotal
                $abonoActual = 0;
                $saldoActual = 0;
            @endphp
            ...
            @forelse($reportes as $r)
                @php
                    ...
                    $abonoVenta = (float)($r->abonos ?? 0);
                    $saldoVenta = (float)($r->saldo ?? 0);

                    $isNewVenta = ($ventaActual === null || $ventaId !== $ventaActual);
                @endphp

                {{-- Cuando cambia la venta: imprimir subtotal de la venta anterior --}}
                @if($ventaActual !== null && $ventaId !== $ventaActual)
                ...
                @php
                    // Si es una venta nueva, actualiza doc + abono/saldo de esa venta (para su futuro subtotal)
                    if ($isNewVenta) {
                        $ventaActual = $ventaId;
                        $documentoActual = $r->documento;

                        $abonoActual = $abonoVenta;
                        $saldoActual = $saldoVenta;

                        // acumular totales generales (una vez por venta)
                        $totAbonos += $abonoVenta;
                        $totSaldo  += $saldoVenta;
                    }

                    // acumular subtotales por venta (detalle)
            ...
            {{-- Subtotal final (última venta) + Total general --}}
            @if(count($reportes) > 0)
                <tr class="subtotal-row">
                    <td>Abono: S/. {{ number_format($abonoActual, 2) }}</td>
                    <td>Saldo: S/. {{ number_format($saldoActual, 2) }}</td>
