{{-- ============================================================
    Fórmulas de Alimento (Cerdos) — Index
            ...
            <div class="card mb-4">
                <div class="card-header d-flex align-items-center">
                    <h3 class="card-title flex-grow-1">
                        <i class="bi bi-piggy-bank me-2"></i>Fórmulas de Alimento — Cerdos
                    </h3>
                    ...
                    @can('formulas_alimento_cerdo_create')
...
@canany(['formulas_alimento_cerdo_create', 'formulas_alimento_cerdo_edit'])
    @include('formulas-alimento-cerdo.formula-action')
@endcanany

@include('formulas-alimento-cerdo.formula-view')
@include('formulas-alimento-cerdo.export-modal')

<div id="modalContainer"></div>
@endsection
@push('scripts')
<script>
class FormulaAlimentoCerdoManager extends CrudManager {
    constructor() {
        super();
        this.formulaId = null;
        this.baseUrl = '{{ url("formulas-alimento-cerdo") }}';
    ...
    initDataTable() {
        this.tabla = $('#formulasTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: { url: '{{ route("formulas-alimento-cerdo.datatable") }}', type: 'GET' },
            columns: [
    ...
    setupEventListeners() {
        ...
        searchInputEl?.addEventListener('keydown', (e) => {
            if (e.key === 'ArrowDown') { e.preventDefault(); self.moveSearchHighlight(1); }
            else if (e.key === 'ArrowUp') { e.preventDefault(); self.moveSearchHighlight(-1); }
            else if (e.key === 'Enter') { e.preventDefault(); self.selectHighlightedIngredient(); }
        });
    ...
    async exportarExcel() {
        ...
        const url = "{{ url('formulas-alimento-cerdo') }}/" + id + "/export/excel";
        try {
            ...
            a.href = downloadUrl;
            a.download = 'formula_cerdo_' + nombre.replace(/\s+/g, '_') + '.xlsx';
    ...
    async exportarPdf() {
        ...
        const url = "{{ url('formulas-alimento-cerdo') }}/" + id + "/export/pdf";
    ...
    showCreateModal() {
        ...
        document.getElementById('formulaTitle').textContent = 'Nueva Fórmula (Cerdos)';
        document.getElementById('method_field_formula').value = '';
        document.getElementById('formFormula').action = '{{ route("formulas-alimento-cerdo.store") }}';
    ...
    async editFormula(id) {
        try {
            ...
            document.getElementById('formulaTitle').textContent = 'Editar Fórmula (Cerdos): ' + f.nombre;
    ...
    async verFormula(id) {
        try {
            ...
            document.getElementById('view_em').textContent = n.em.toFixed(4);
            document.getElementById('view_fdn').textContent = n.fdn.toFixed(4);
            document.getElementById('view_fibra').textContent = n.fibra.toFixed(4);
            document.getElementById('view_fda').textContent = n.fda.toFixed(4);
            document.getElementById('view_grasa').textContent = n.grasa.toFixed(4);
            document.getElementById('view_calcio').textContent = n.calcio.toFixed(4);
            document.getElementById('view_fosforo').textContent = n.fosforo.toFixed(4);
            document.getElementById('view_magnesio').textContent = n.magnesio.toFixed(4);
            ...
            document.getElementById('view_ceniza').textContent = n.ceniza.toFixed(4);
            document.getElementById('view_lactosa').textContent = n.lactosa.toFixed(4);
            document.getElementById('view_lisina').textContent = n.lisina.toFixed(4);
            document.getElementById('view_metionina').textContent = n.metionina.toFixed(4);
            document.getElementById('view_treonina').textContent = n.treonina.toFixed(4);
    ...
    async buscarIngredientes(q) {
        ...
        try {
            const items = await this.fetchData(`{{ route('formulas-alimento-cerdo.ingredientes.buscar') }}?q=${encodeURIComponent(q)}`);
            ...
            items.forEach((it, index) => {
                ...
                if (!yaExiste) {
                    li.addEventListener('mouseenter', () => {
                        if (this.searchHighlightedIndex !== index) li.style.backgroundColor = '#cfe2ff';
                    });
                    li.addEventListener('mouseleave', () => {
                        if (this.searchHighlightedIndex !== index) li.style.backgroundColor = '#fff';
                    });
    ...
    updateSearchHighlight() {
        this.searchResultsElements.forEach((el, i) => {
            if (i === this.searchHighlightedIndex) {
                ...
                el.style.color = '#fff';
                el.querySelectorAll('strong, .badge, small').forEach(child => { child.style.color = '#fff'; });
                el.scrollIntoView({ block: 'nearest' });
            } else {
                el.style.backgroundColor = '#fff';
                el.style.color = '';
                el.querySelectorAll('strong, .badge, small').forEach(child => { child.style.color = ''; });
            }
        });
    }
    ...
    removeIngredienteFromTable(ingredienteId) {
        this.ingredientesEnTabla = this.ingredientesEnTabla.filter(x => x.ingrediente_id != ingredienteId);
        this.renderIngredientesTable();
        this.recalcular();
    }
    ...
    updateRowCosto(index) {
        ...
        if (costoCells[index]) costoCells[index].textContent = costo;
    }
    ...
    recalcular() {
        ...
        let nutrientes = {
            materia_seca: 0, proteina_cruda: 0, em: 0, fdn: 0, fibra: 0, fda: 0,
            grasa: 0, calcio: 0, fosforo: 0, magnesio: 0,
            almidon: 0, azucar: 0, ceniza: 0, lactosa: 0,
            lisina: 0, metionina: 0, treonina: 0,
        };
        ...
        this.ingredientesEnTabla.forEach(ing => {
            ...
            if (nutData) {
                for (const campo in nutrientes) {
                    nutrientes[campo] += (cantKg * parseFloat(nutData[campo] || 0)) / 1000;
                }
            }
        });
        ...
        if (gananciaEl) gananciaEl.style.color = gananciaSaco >= 0 ? '#198754' : '#dc3545';

        this.setText('nut_ms', nutrientes.materia_seca.toFixed(4));
        this.setText('nut_pc', nutrientes.proteina_cruda.toFixed(4));
        this.setText('nut_em', nutrientes.em.toFixed(4));
        this.setText('nut_fdn', nutrientes.fdn.toFixed(4));
        this.setText('nut_fibra', nutrientes.fibra.toFixed(4));
        this.setText('nut_fda', nutrientes.fda.toFixed(4));
        this.setText('nut_grasa', nutrientes.grasa.toFixed(4));
        this.setText('nut_calcio', nutrientes.calcio.toFixed(4));
        this.setText('nut_fosforo', nutrientes.fosforo.toFixed(4));
        this.setText('nut_magnesio', nutrientes.magnesio.toFixed(4));
        this.setText('nut_almidon', nutrientes.almidon.toFixed(4));
        this.setText('nut_azucar', nutrientes.azucar.toFixed(4));
        this.setText('nut_ceniza', nutrientes.ceniza.toFixed(4));
        this.setText('nut_lactosa', nutrientes.lactosa.toFixed(4));
        this.setText('nut_lisina', nutrientes.lisina.toFixed(4));
        this.setText('nut_metionina', nutrientes.metionina.toFixed(4));
        this.setText('nut_treonina', nutrientes.treonina.toFixed(4));
    }
...
document.addEventListener('DOMContentLoaded', () => {
    window.formulaAlimentoCerdoManager = new FormulaAlimentoCerdoManager();
    document.getElementById('mnuNutricion')?.classList.add('menu-open');
    document.getElementById('itemFormulasAlimentoCerdo')?.classList.add('active');
});
</script>
@endpush
